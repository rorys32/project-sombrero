...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("parse_acid_alerts.log"), logging.StreamHandler()]
)
...
# Output files
json_output = "data/sanitized/acid_alerts_cleaned.json"
csv_output = "data/sanitized/acid_alerts_cleaned.csv"
os.makedirs("data/sanitized", exist_ok=True)

# Define the alert keyword for Acid
ACID_KEYWORD = "AI hallucination"
...
for i, message in enumerate(mbox):
    ...
    # Filter for alerts containing "AI hallucination"
    normalized_alert_term = alert_term.lower().replace('"', '')
    if ACID_KEYWORD.lower() not in normalized_alert_term:
        logging.info(f"Skipping alert: '{alert_term}' does not contain '{ACID_KEYWORD}'")
        continue
    ...
    for link in links:
        ...
        if ACID_KEYWORD.lower() in synopsis.lower():
            synopsis = synopsis.replace(ACID_KEYWORD, f"{ACID_KEYWORD} ")
...
# Log summary
logging.info(f"Summary: Evaluated {total_emails} total emails, found {total_google_alerts} Google Alerts, saved {len(alerts_data)} '{ACID_KEYWORD}' alerts")

# Save to JSON
with open(json_output, "w", encoding="utf-8") as json_file:
    json.dump(alerts_data, json_file, indent=4, ensure_ascii=False)
logging.info(f"Saved {len(alerts_data)} '{ACID_KEYWORD}' alerts to {json_output}")
...
logging.info(f"Saved {len(alerts_data)} '{ACID_KEYWORD}' alerts to {csv_output}")