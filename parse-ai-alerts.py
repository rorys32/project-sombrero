...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("parse_ai_alerts.log"), logging.StreamHandler()]
)
...
# Output files
json_output = "data/sanitized/ai_google_alerts_cleaned.json"
csv_output = "data/sanitized/ai_google_alerts_cleaned.csv"
os.makedirs("data/sanitized", exist_ok=True)

# Define the alert keyword for Sombrero
SOMBRERO_KEYWORD = "that uses AI to"
...
for i, message in enumerate(mbox):
    ...
    # Filter for alerts containing "that uses AI to"
    normalized_alert_term = alert_term.lower().replace('"', '')
    if SOMBRERO_KEYWORD.lower() not in normalized_alert_term:
        logging.info(f"Skipping alert: '{alert_term}' does not contain '{SOMBRERO_KEYWORD}'")
        continue
    ...
    for link in links:
        ...
        # Clean up data and fix spacing before "that uses AI to"
        title = html.unescape(title)
        synopsis = html.unescape(synopsis)
        keyword_lower = SOMBRERO_KEYWORD.lower()
        if keyword_lower in synopsis.lower():
            # Replace only if not at the start and no space precedes
            pos = synopsis.lower().find(keyword_lower)
            if pos > 0 and synopsis[pos-1] != " ":
                synopsis = synopsis[:pos] + " " + synopsis[pos:]
            synopsis = synopsis.replace(SOMBRERO_KEYWORD, f"{SOMBRERO_KEYWORD} ")
...
# Log summary
logging.info(f"Summary: Evaluated {total_emails} total emails, found {total_google_alerts} Google Alerts, saved {len(alerts_data)} '{SOMBRERO_KEYWORD}' alerts")

# Save to JSON
with open(json_output, "w", encoding="utf-8") as json_file:
    json.dump(alerts_data, json_file, indent=4, ensure_ascii=False)
logging.info(f"Saved {len(alerts_data)} '{SOMBRERO_KEYWORD}' alerts to {json_output}")
...
logging.info(f"Saved {len(alerts_data)} '{SOMBRERO_KEYWORD}' alerts to {csv_output}")